...
inline double integrate(const Eigen::VectorXd& vector, int dims) {
    double i=0.0;
    double integral=0;
    while(i<(dims-0.001)){
        integral += 0.001/6 * (vector(i)+vector(i+0.001)+4*vector(0.5*(i+i+0.001)));
        i += 0.001;
    }
    return integral;
}
...
inline Eigen::MatrixXd generateInertiaM(Eigen::MatrixXd B, int size){
